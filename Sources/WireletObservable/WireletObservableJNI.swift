    ...
    public enum WireletObservableJNI {
        ...
        public static func unwrap<T: AnyObject>(_ pointer: jlong, as: T.Type = T.self) -> T {
            guard let opaque = UnsafeRawPointer(bitPattern: Int(pointer)) else {
                preconditionFailure("unwrap called with a null self pointer")
            }
        ...
        public static func release<T: AnyObject>(_ pointer: jlong, as: T.Type = T.self) {
            guard let opaque = UnsafeRawPointer(bitPattern: Int(pointer)) else { return }
