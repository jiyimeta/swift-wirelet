...
enum ViewModelEmitter {
    ...
    static func emit(
        ...
        let membersBlock = membersBlock(plans: plans, methodRenders: methodRenders)
        let externalsBlock = externalsBlock(plans: plans, methodRenders: methodRenders)
        ...
        \(membersBlock)    override fun onCleared() {
                nativeRelease(nativePtr)
                super.onCleared()
            }

        \(externalsBlock)    private external fun nativeRelease(self: Long)
    ...
    /// The combined public-member block: StateFlow members, track helpers,
    /// setter wrappers and method wrappers, each as a trailing-padded block.
    private static func membersBlock(
        plans: [(ObservableProperty, ObservableKotlinTypeMap.Plan)],
        methodRenders: [(publicFn: String, externalDecl: String, extraImports: Set<String>)],
    ) -> String {
        ...
        return joinBlock(stateFlowMembers)
            + joinBlock(trackHelpers)
            + joinBlock(setterWrappers)
            + joinBlock(methodWrappers)
    }

    /// The combined `external fun` declaration block: track, setter and
    /// method externals, each as a non-trailing-padded block.
    private static func externalsBlock(
        plans: [(ObservableProperty, ObservableKotlinTypeMap.Plan)],
        methodRenders: [(publicFn: String, externalDecl: String, extraImports: Set<String>)],
    ) -> String {
        let externalTrackDecls = plans.map { prop, plan in
            externalTrackDecl(property: prop, plan: plan)
        }.joined(separator: "\n")
        ...
        return joinBlock(externalTrackDecls, trailing: false)
            + joinBlock(setterExternalDecls, trailing: false)
            + joinBlock(methodExternalDecls, trailing: false)
    }

    private static func externalTrackDecl(
        property: ObservableProperty,
        plan: ObservableKotlinTypeMap.Plan,
    ) -> String {
        let fnName = nativeTrackFnName(property)
        return "    private external fun \(fnName)(self: Long, onChange: Runnable): \(plan.nativeTrackReturn)"
    }
    ...
    private static func methodWrapper(
        ...
        let publicFn = """
            fun \(method.name)(\(argDecls.joined(separator: ", ")))\(retClause) =
                \(body)
        """
        let joinedParams = externalParams.joined(separator: ", ")
        let external = "    private external fun \(nativeFn)(\(joinedParams))\(externalRetClause)"
