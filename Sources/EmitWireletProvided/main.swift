...
struct CLIArguments {
    ...
    /// Filters generated files by the Kotlin package of their relative path.
    /// Multi-module Gradle builds use this to assign disjoint interface packages
    /// to disjoint modules from one schema source.
...
guard let args = CLIArguments.parse(CommandLine.arguments) else {
    writeStderr("""
    usage: emit-wirelet-provided --config <file> --source <dir> --output <dir> \
...
let config = try JSONDecoder().decode(ProvidedCodegenConfig.self, from: configData)

let sourceURL = URL(fileURLWithPath: args.sourceDir, isDirectory: true)
var aggregateSchema = ProvidedSchema(services: [])
if let enumerator = FileManager.default.enumerator(
    ...
    for case let url as URL in enumerator {
        ...
        aggregateSchema.services.append(contentsOf: schema.services)
    }
}

aggregateSchema.services.sort(by: { $0.name < $1.name })

let emitter = ProvidedKotlinEmitter(config: config)
